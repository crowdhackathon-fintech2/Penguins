...
contract Ouroboros {
	 ...
	 * @dev Normally the Ourobros contract would be deployed on the network by a seperate
	 *      one time contract agreed upon by the two initial banks complying with the protocol.
	 */
	...
	/**
	 * @notice Bank addition override for presentation
	 */
	function addBankAddress(address _bank) public {
	    ouroborosCompatibleBanks[_bank] = true;
	    bankAddresses.push(_bank);
	}
	
	/**
	 * @notice Fee override for presentation
	 */
	function feeUpdate(uint256 feePercentage) public {
	    ouroborosFee = feePercentage;
	}
